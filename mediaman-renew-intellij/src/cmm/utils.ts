...
export class Checker {
    ...
    static evs(element: HTMLInputElement | string, name: string): string {
    ...
    static evn(element: HTMLInputElement | string, name: string): number {
        const stringValue = this.evs(element, name);
        const numberValue = Number(stringValue);
        if(isNaN(numberValue))
            throw new Error(`Can't get numeric value from ${ name }`);
