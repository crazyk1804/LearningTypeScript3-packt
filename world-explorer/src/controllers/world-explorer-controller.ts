import { UnreachableCaseError } from "../common/errors/error";
import { Country, DataPoint } from "../domain";
import { PopulationService } from "../services/population-service.intf";
import { WorldBankApiV2CountryInformation, WorldBankApiV2Indicators } from "../services/world-bank-api";
...
export class WorldExplorerControllerImpl implements WorldExplorerController {
    ...
    constructor(populationService: PopulationService, view: WorldExplorerView) {
        if(!populationService) {
            throw new Error('The poulationservice is mandatory');
        }
        if(!view) {
        ...
        this.loadCountries()
            ...
            .catch(error => {
                ...
                this._view.displayErrorMessage('WorldExplorer failed to load. Plaese contact IT support! ;-)');
            });
        this.loadYears();
    }
    ...
    async loadCountries(): Promise<void> {
        ...
        try {
            ...
            this._view.displayErrorMessage(
                'Could not load the list of countries! Are you connected to the internet?'
            );
        }
    }
    ...
    async renderChart(): Promise<void> {
        ...
        if(this._countriesMap.has(chartFormDetails.countryId)) {
            ...
            console.error([
                'Tried to render a chart for an unknown country. ',
                'Identifier: ',
        ...
        const indicator: WorldBankApiV2Indicators = WorldBankApiV2Indicators[
            chartFormDetails.indicator as keyof typeof WorldBankApiV2Indicators
        ]; 
        if(!indicator) {
            console.error([
                'Tried to render a chart for an unknown indicator. ',
                'Identifier: ',
                chartFormDetails.indicator
            ].join(''));
            this._view.displayErrorMessage('Failed to render the chart!');
        ...
        if(chartFormDetails.fromYear > chartFormDetails.toYear) {
            console.error([
                'Tried to render a chart for an invalid date range: from ',
                '[', chartFormDetails.fromYear, '] to',
                '[', chartFormDetails.toYear, ']'
            ].join(''));
            this._view.displayErrorMessage('The start year cannot be after the end year');
            return;
        }
        let dateRange: string = chartFormDetails.fromYear.toString();
        if(chartFormDetails.fromYear !== chartFormDetails.toYear) {
        ...
        try {
            ...
            switch(indicator) {
                ...
                case WorldBankApiV2Indicators.ADULT_FEMALE_SURVIVAL_TO_65:
                    ...
                    break;
                default:
                    throw new UnreachableCaseError(indicator); // will fall to compile if we forget a switch case
            }

            const dataLabel = [
                title, ' in ', country.name, ' for the ', 
                chartFormDetails.fromYear, '-', chartFormDetails.toYear,
                ' time period'
            ].join('');
