class TodoItem {
    ...
    constructor(private _description: string, identifier?: string) {
        ...
        if(identifier) {
            ...
            // this is just for the example: for any real prject, use
...
class TodoList {
    ...
    constructor(todoList?: TodoItem[]) {
        // frist we make sure that we have received a valid array
...
class HTMLTodoListView implements TodoListView {
    ...
    constructor() {
        ...
        if(!this.todoListDiv) {
            throw new Error('Could not find the todoListContainer HTML div. Is the HTML corrent?');
        }

        if(!this.todoListFilter) {
            throw new Error('Could not find the todoFilter HTML input element. Is the HTML connrect?');
        }
    }
...
class TodoIt implements TodoListController {
    ...
    addTodo(): void {
        ...
        // verify that ther is something to add
